...
(function (jQuery){
    var keep = false;
    jQuery.jAlert = function(msg, subject, opts){
        var options = { time: 5233, speed: 'slow', css: { fontSize:14, color:'#333'} };
        if(opts)jQuery.extend(options, opts);
        jQuery.extend(options.css, {
            borderStyle: 'solid',
            borderWidth: '1px',
            padding: '1px',
            display: 'block'
        ...
        var panel =   jQuery('<div>').css(options.css).css({ backgroundColor: '#F3F3F3', borderColor: '#FBFBFB', right: '3px', bottom: '-33px',position: 'absolute', opacity: '0.33',width: 'auto', height: 'auto', zIndex:999999999 });
        var shadow =  jQuery('<div>').css(options.css).css({ backgroundColor: '#D8D8D8', borderColor: '#E8E8E8' });
        var border =  jQuery('<div>').css(options.css).css({ backgroundColor: '#FFF', borderColor: '#BBB', padding: '10px' });
        var title =   jQuery('<div>').css({ display: 'block', fontWeight:'bold'}).text(subject);
        var message = jQuery('<div>').css({ display: 'block', marginTop:'10px', lineHeight:(options.css.fontSize*2)+'px' }).html(msg);
        border.append(title).append(message);
        panel.append(shadow.append(border));
        jQuery('body').append(panel);
        panel.mousemove(function(){keep = true;}).mouseout(function(){keep = false;});
        panel.animate({ bottom: '0px', opacity: '1' }, options.speed, null, function(){
            var close_action = function(){
                panel.animate({ bottom: '-133px', opacity: '0.33' }, options.speed, null, function(){
                    panel.remove();
                });
            };
            var close_button = jQuery('<img>').attr('src', 'css/images/close.png').css({float: 'right', cursor:'pointer'});
            title.append(close_button.click(close_action));
            var auto = function(){
                if(!keep){
                    setTimeout(close_action, 3333);
                }else{
                    setTimeout(auto, 3333);
                }
            };
            auto();
        });
